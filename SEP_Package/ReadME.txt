This ReadMe explains the requirements and getting started to run the solar energetic particles (SEP) prediction using the Deep Learning network SEP BiLSTM.

Prerequisites:

Python, Tensorflow, and Cuda:
The initial work and implementation of the SEP BiLSTM was done using Python version 3.9.7, Tensorflow 2.6.0 and GPU Cuda version cuda_11.4.r11.4.
...
Note: There is a requirements file already created for you to use that includes all packages with their versions. 
       The files are located in the root directory of the SEP_Package package.
...
After downloading the files from github repository: https://github.com/deepsuncode/SEP-prediction the SEP_Package package includes the following folders and files:
 ...
 default_models                - includes default trained model used during the initial work of SEP BiLSTM.
 logs                          - includes the logging information.
 data                          - includes a list of SEP data sets that can be used for training and prediction.
 results                       - will include the prediction result file(s).
 SEP_test.py                   - Python program to test/predict a trained model.
 SEP_train.py                  - Python program to train a model and save it to models directory.
 Other files are included as utilities files for training and testing.
 
Running a Test/Prediction Task:
To run a test/prediction, you should use the existing data sets from "data" directory. 
...
Note that the testing task will use the models form the default_models directory if you did not run a training task for the specified classification and time window
...
Running a Training Task:
	...
	provide a classification type and a  time window, for example 4, to h=4 ahead hours, save the trained model to the "model" directory.
	Available classification types: FC_S and F_S
	Available time window are: 12, 24, 36, 48, 60, and 72
